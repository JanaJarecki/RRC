...
stocks <- fread("../../data/study2_stocks/risk_international_ROI.csv")
stocks
stocks[, index := gsub("EuroStoxx50", "ES50", index)]
stocks[, index := gsub("jse", "JSE", index)]
stocks[, index := gsub("Dow Jones", "DJIA", index)]
stocks[, index := gsub("ALL ORDINARIES", "ASX", index)]
stocks[, index := gsub("MERVAL", "MERV", index)]
stocks[, index := gsub("FTSE STRAITS TIMES", "FSSTI", index)]
stocks[, index := gsub("Hang Seng", "HSI", index)]
stocks[, index := gsub("Nikkei", "Nikkei225", index)]
stocks[, index := gsub("OMX STOCKHOLM", "OMXS30", index)]
stocks[, index := gsub("RTS", "RTSI", index)]
stocks[, index := gsub("Shanghai Composite", "SSEC", index)]
stocks[, index := gsub("Sensex", "BSE", index)]
stocks[, index := gsub("S&P TSX COMPOSITE", "TSX", index)]
stocks[, index := gsub("JAKARTA COMPOSITE", "IDX", index)]
# stocks[, index := gsub("HSI", "", index)]
stocks[, index := gsub("FTSE MALAYSIA", "KLSE", index)]
#stocks[, index := gsub("FTSE JSE ALL SHARE", "", index)]
stocks[, index := gsub("SMI", "SMI20", index)]
stocks[, index := gsub("FTSE", "FTSE100", index)]
stocks[, roi := round(pmax(pmin(ROIyearly, 1), -1), 2)]
stocks[, date := fasttime::fastPOSIXct(date)]
stocks[, year := year(date)]
stocks[, range(year)]
stocks2 <- stocks[year >= 2007 & year <= 2016]
d[!duplicated(index), .(index, mroi_obj) ][stocks2[, mean(roi, na.rm = T), by = index], on = "index"]


stocks2[, mean(roi, na.rm = T), by = index][d[!duplicated(index), .(index, mroi_obj)], on = "index"]
